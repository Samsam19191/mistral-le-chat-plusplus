...
function isAlternativesList(items: string[]): boolean {
  ...
  if (items.length === 2) {
    const [first, second] = items.map(item => item.toLowerCase().trim());
    const numberWords = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten'];
    const hasNumber = items.some(item => /^\d+$/.test(item.trim()));
    const hasNumberWord = items.some(item => numberWords.includes(item.toLowerCase().trim()));
    if (hasNumber && hasNumberWord) return true;
  }
  
  const allSimple = items.every(item => /^\w+$/.test(item.trim()));
  if (allSimple && items.length > 4) return true;
